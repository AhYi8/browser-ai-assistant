...
* 引导跟进只在当前任务后续工具决策点或最终总结前注入；如果当前任务没有可消费的后续决策点，必须保留其用户消息引用并在任务结束后继续发送这条既有用户消息，避免用户补充丢失且不重新显示到排队列表。
...
* 聊天偏好和当前聊天设置中的 `maxTokens` 语义是“最大聊天上下文”，只用于请求上下文预算、页面上下文裁剪和自动压缩阈值判断，不得主动作为模型输出 `max_tokens` 参数；模型输出上限只能来自模型配置或后续独立字段。
...
* 当前上下文 Token 数展示与自动压缩触发必须复用同一估算口径：优先参考最新成功聊天/工具最终回答的真实 response usage 输入规模，再对其后的新增消息使用本地保守估算；不得回退为会话累计 Token 用量。
