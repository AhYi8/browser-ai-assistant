import { useId, useState } from "react";
import type { ReactNode } from "react";
import {
  MODEL_TOOL_CAPABILITY_VALUES,
  MODEL_TOOL_RISK_VALUES,
  MODEL_TOOL_RUNTIME_VALUES,
  TAVILY_SEARCH_TOOL_ID,
  filterModelToolsByClassification,
  getModelToolGroups,
  getRegisteredModelTools,
} from "../../../shared/models/toolRegistry";
import type { ModelToolRegistryEntry } from "../../../shared/models/types";
import type { ModelToolCapability, ModelToolRisk, ModelToolRuntimeRequirement } from "../../../shared/models/types";
import type { BrowserAutomationMode } from "../../../shared/toolAuthorization";
import type { ChatPreferenceValues, SendShortcut } from "../../../shared/types";
import { isModelToolConfigured } from "../../../shared/webSearch/toolAvailability";
import { useAppStore } from "../../state/appStore";
import { useComposedTextInput } from "../useComposedTextInput";
import { GlobalPreferenceNumberInput } from "./GlobalPreferenceNumberInput";

type ChatPreferenceSectionId = "common" | "prompts" | "tools" | "advanced";

const sendShortcutOptions: Array<{ value: SendShortcut; label: string }> = [
  { value: "enter", label: "Enter" },
  { value: "shift_enter", label: "Shift+Enter" },
  { value: "ctrl_enter", label: "Ctrl+Enter" },
  { value: "alt_enter", label: "Alt+Enter" },
];

const followUpBehaviorOptions: Array<{ value: ChatPreferenceValues["followUpBehavior"]; label: string }> = [
  { value: "queue", label: "排队" },
  { value: "guide", label: "引导" },
];

const browserAutomationModeOptions: Array<{ value: BrowserAutomationMode; label: string }> = [
  { value: "normal_restricted", label: "普通模式" },
  { value: "controlled_enhanced", label: "受控增强" },
  { value: "full_access", label: "完全访问" },
];

const toolRuntimeLabels: Record<ModelToolRuntimeRequirement, string> = {
  local: "本地工具",
  external_web: "公开网页搜索",
  browser_control: "浏览器控制",
  controlled_enhanced: "受控增强",
  full_access: "完全访问",
  mcp_remote: "MCP 远程工具",
};

const toolCapabilityLabels: Record<ModelToolCapability, string> = {
  observe_page: "观察页面",
  operate_page: "操作页面",
  analyze_site: "分析现场",
  confirm_boundary: "请求确认",
  deliver_result: "交付结果",
  search_public_web: "公开搜索",
  system_context: "系统上下文",
  call_remote_tool: "调用远程工具",
};

const toolRiskLabels: Record<ModelToolRisk, string> = {
  low: "低风险",
  medium: "中风险",
  high: "高风险",
  critical: "最高风险",
};

export function ChatPreferenceSettings() {
  const [expandedSections, setExpandedSections] = useState<Set<ChatPreferenceSectionId>>(() => new Set());
  const [toolPermissionsExpanded, setToolPermissionsExpanded] = useState(false);
  const [toolQuery, setToolQuery] = useState("");
  const [enabledToolsOnly, setEnabledToolsOnly] = useState(false);
  const [runtimeFilter, setRuntimeFilter] = useState<ModelToolRuntimeRequirement | "">("");
  const [capabilityFilter, setCapabilityFilter] = useState<ModelToolCapability | "">("");
  const [riskFilter, setRiskFilter] = useState<ModelToolRisk | "">("");
  const toolPermissionsContentId = useId();
  const toolPermissionsSummaryId = useId();
  const chatPreferences = useAppStore((state) => state.chatPreferences);
  const mcpSettings = useAppStore((state) => state.mcpSettings);
  const webSearchSettings = useAppStore((state) => state.webSearchSettings);
  const updateChatPreferences = useAppStore((state) => state.updateChatPreferences);
  const registeredTools = getRegisteredModelTools(mcpSettings);
  const classifiedTools = filterModelToolsByClassification(registeredTools, {
    ...(runtimeFilter ? { runtime: runtimeFilter } : {}),
    ...(capabilityFilter ? { capability: capabilityFilter } : {}),
    ...(riskFilter ? { risk: riskFilter } : {}),
  });
  const normalizedToolQuery = toolQuery.trim().toLocaleLowerCase();
  const enabledToolIds = new Set(chatPreferences.enabledToolIds);
  const isToolEnabled = (tool: ModelToolRegistryEntry) => isModelToolConfigured(tool, webSearchSettings) && enabledToolIds.has(tool.id);
  const filteredTools = classifiedTools.filter((tool) => {
    const matchesQuery = normalizedToolQuery
      ? [getToolDisplayName(tool), tool.name, tool.description]
          .filter(Boolean)
          .some((value) => value?.toLocaleLowerCase().includes(normalizedToolQuery))
      : true;
    return matchesQuery && (!enabledToolsOnly || isToolEnabled(tool));
  });
  const registeredToolGroups = getModelToolGroups(filteredTools);
  const enabledToolCount = registeredTools.filter(isToolEnabled).length;
  const filteredEnableableToolIds = filteredTools
    .filter((tool) => isModelToolConfigured(tool, webSearchSettings) && !enabledToolIds.has(tool.id))
    .map((tool) => tool.id);
  const filteredEnabledToolIds = filteredTools.filter(isToolEnabled).map((tool) => tool.id);
  const systemPromptInput = useComposedTextInput(chatPreferences.systemPrompt, (systemPrompt) => {
    void updateChatPreferences({ systemPrompt });
  });
  const contextCompressionPromptInput = useComposedTextInput(chatPreferences.contextCompressionPrompt, (contextCompressionPrompt) => {
    void updateChatPreferences({ contextCompressionPrompt });
  });

  const toggleSection = (sectionId: ChatPreferenceSectionId) => {
    setExpandedSections((current) => {
      const next = new Set(current);
      if (next.has(sectionId)) {
        next.delete(sectionId);
      } else {
        next.add(sectionId);
      }
      return next;
    });
  };
  const handleToolToggle = (toolId: string, checked: boolean) => {
    const tool = registeredTools.find((item) => item.id === toolId);
    if (tool && !isModelToolConfigured(tool, webSearchSettings)) {
      return;
    }
    const nextToolIds = checked ? [...chatPreferences.enabledToolIds, toolId] : chatPreferences.enabledToolIds.filter((id) => id !== toolId);
    void updateChatPreferences({ enabledToolIds: Array.from(new Set(nextToolIds)) });
  };
  const handleEnableFilteredTools = () => {
    void updateChatPreferences({
      enabledToolIds: Array.from(new Set([...chatPreferences.enabledToolIds, ...filteredEnableableToolIds])),
    });
  };
  const handleDisableFilteredTools = () => {
    const filteredIds = new Set(filteredTools.map((tool) => tool.id));
    void updateChatPreferences({ enabledToolIds: chatPreferences.enabledToolIds.filter((toolId) => !filteredIds.has(toolId)) });
  };

  return (
    <section className="chat-preference-layout" aria-label="聊天偏好">
      <h3 className="text-base font-semibold">聊天偏好</h3>

      <PreferenceSection
        title="常用设置"
        summary={`发送：${getOptionLabel(sendShortcutOptions, chatPreferences.sendShortcut)} · 跟进：${getOptionLabel(followUpBehaviorOptions, chatPreferences.followUpBehavior)}`}
        expanded={expandedSections.has("common")}
        onToggle={() => toggleSection("common")}
      >
        <div className="chat-preference-primary-grid">
          <label className="chat-preference-field">
            发送快捷键
            <select
              className="ui-input chat-preference-shortcut-select"
              aria-label="发送快捷键"
              value={chatPreferences.sendShortcut}
              onChange={(event) => void updateChatPreferences({ sendShortcut: event.target.value as SendShortcut })}
            >
              {sendShortcutOptions.map((option) => (
                <option key={option.value} value={option.value}>{option.label}</option>
              ))}
            </select>
          </label>
          <label className="chat-preference-field">
            跟进行为
            <select
              className="ui-input chat-preference-shortcut-select"
              aria-label="跟进行为"
              value={chatPreferences.followUpBehavior}
              onChange={(event) => void updateChatPreferences({ followUpBehavior: event.target.value as ChatPreferenceValues["followUpBehavior"] })}
            >
              {followUpBehaviorOptions.map((option) => (
                <option key={option.value} value={option.value}>{option.label}</option>
              ))}
            </select>
            <span className="ui-muted text-xs">运行中发送草稿时使用；Ctrl+Shift+Enter 会执行相反操作。</span>
          </label>
        </div>
        <div className="chat-preference-switch-grid">
          <PreferenceSwitch
            checked={chatPreferences.historyDrawerDefaultOpen}
            label="默认展开左侧历史面板"
            onChange={(checked) => void updateChatPreferences({ historyDrawerDefaultOpen: checked })}
          />
          <PreferenceSwitch
            checked={chatPreferences.injectPageContextByDefault}
            label="新对话默认注入当前页面上下文"
            onChange={(checked) => void updateChatPreferences({ injectPageContextByDefault: checked })}
          />
          <PreferenceSwitch
            checked={chatPreferences.extractHtmlByDefault}
            label="新对话默认提取 HTML 源码"
            onChange={(checked) => void updateChatPreferences({ extractHtmlByDefault: checked })}
          />
        </div>
      </PreferenceSection>

      <PreferenceSection
        title="提示词与压缩"
        summary={`系统提示词 ${getTextLengthLabel(chatPreferences.systemPrompt)} · 压缩 Prompt ${getTextLengthLabel(chatPreferences.contextCompressionPrompt)}`}
        expanded={expandedSections.has("prompts")}
        onToggle={() => toggleSection("prompts")}
      >
        <label className="grid gap-1 text-sm">
          系统提示词
          <textarea className="ui-input min-h-32" aria-label="全局系统提示词" {...systemPromptInput} />
        </label>
        <label className="grid gap-1 text-sm">
          上下文压缩 Prompt
          <textarea className="ui-input min-h-32" aria-label="全局上下文压缩 Prompt" {...contextCompressionPromptInput} />
        </label>
      </PreferenceSection>

      <PreferenceSection
        title="工具调用"
        summary={`${chatPreferences.toolCallingEnabled ? "已启用" : "已关闭"} · 已启用 ${enabledToolCount} / ${registeredTools.length} 个工具`}
        expanded={expandedSections.has("tools")}
        onToggle={() => toggleSection("tools")}
      >
        <PreferenceSwitch
          checked={chatPreferences.toolCallingEnabled}
          label="启用工具调用"
          onChange={(checked) => void updateChatPreferences({ toolCallingEnabled: checked })}
        />
        <fieldset className="chat-preference-control-fieldset" disabled={!chatPreferences.toolCallingEnabled}>
          <div className="chat-preference-tool-settings-grid">
            <label className="chat-preference-field">
              浏览器自动化默认模式
              <select
                className="ui-input chat-preference-shortcut-select"
                aria-label="浏览器自动化默认模式"
                value={chatPreferences.defaultBrowserAutomationMode ?? "normal_restricted"}
                onChange={(event) => void updateChatPreferences({ defaultBrowserAutomationMode: event.target.value as BrowserAutomationMode })}
              >
                {browserAutomationModeOptions.map((option) => (
                  <option key={option.value} value={option.value}>{option.label}</option>
                ))}
              </select>
            </label>
            <label className="chat-preference-field">
              工具调用展示方式
              <select
                className="ui-input chat-preference-shortcut-select"
                aria-label="工具调用展示方式"
                value={chatPreferences.toolCallDisplayMode}
                onChange={(event) => void updateChatPreferences({ toolCallDisplayMode: event.target.value as ChatPreferenceValues["toolCallDisplayMode"] })}
              >
                <option value="assistant_grouped">AI 回复与工具分组</option>
                <option value="compact">紧凑工具过程</option>
              </select>
            </label>
          </div>
          <PreferenceSwitch
            checked={chatPreferences.showToolCallProcessInAssistantMode}
            label="非紧凑模式显示工具调用过程"
            onChange={(checked) => void updateChatPreferences({ showToolCallProcessInAssistantMode: checked })}
          />
        </fieldset>

        <button
          className="chat-preference-permission-toggle"
          type="button"
          aria-label={`${toolPermissionsExpanded ? "收起" : "展开"}工具权限`}
          aria-controls={toolPermissionsContentId}
          aria-describedby={toolPermissionsSummaryId}
          aria-expanded={toolPermissionsExpanded}
          onClick={() => setToolPermissionsExpanded((expanded) => !expanded)}
        >
          <span>
            <span className="chat-preference-permission-title">工具权限</span>
            <span id={toolPermissionsSummaryId} className="chat-preference-section-summary">已启用 {enabledToolCount} / {registeredTools.length}</span>
          </span>
          <span className="chat-preference-section-chevron" aria-hidden="true" />
        </button>

        <div
          id={toolPermissionsContentId}
          className="chat-preference-permission-content"
          role="region"
          aria-label="工具权限"
          hidden={!toolPermissionsExpanded}
        >
            <fieldset className="chat-preference-control-fieldset" disabled={!chatPreferences.toolCallingEnabled}>
              <div className="chat-preference-tool-filter-grid">
                <label className="chat-preference-field chat-preference-tool-search">
                  搜索工具
                  <input
                    className="ui-input"
                    type="search"
                    aria-label="搜索工具"
                    value={toolQuery}
                    onChange={(event) => setToolQuery(event.target.value)}
                  />
                </label>
                <label className="chat-preference-field">
                  能力
                  <select
                    className="ui-input chat-preference-shortcut-select"
                    aria-label="工具能力筛选"
                    value={capabilityFilter}
                    onChange={(event) => setCapabilityFilter(event.target.value as ModelToolCapability | "")}
                  >
                    <option value="">全部能力</option>
                    {MODEL_TOOL_CAPABILITY_VALUES.map((capability) => (
                      <option key={capability} value={capability}>{toolCapabilityLabels[capability]}</option>
                    ))}
                  </select>
                </label>
                <label className="chat-preference-field">
                  运行要求
                  <select
                    className="ui-input chat-preference-shortcut-select"
                    aria-label="工具运行要求筛选"
                    value={runtimeFilter}
                    onChange={(event) => setRuntimeFilter(event.target.value as ModelToolRuntimeRequirement | "")}
                  >
                    <option value="">全部运行要求</option>
                    {MODEL_TOOL_RUNTIME_VALUES.map((runtime) => (
                      <option key={runtime} value={runtime}>{toolRuntimeLabels[runtime]}</option>
                    ))}
                  </select>
                </label>
                <label className="chat-preference-field">
                  风险
                  <select
                    className="ui-input chat-preference-shortcut-select"
                    aria-label="工具风险筛选"
                    value={riskFilter}
                    onChange={(event) => setRiskFilter(event.target.value as ModelToolRisk | "")}
                  >
                    <option value="">全部风险</option>
                    {MODEL_TOOL_RISK_VALUES.map((risk) => (
                      <option key={risk} value={risk}>{toolRiskLabels[risk]}</option>
                    ))}
                  </select>
                </label>
              </div>
              <div className="chat-preference-tool-toolbar">
                <label className="chat-preference-tool-enabled-filter">
                  <input
                    type="checkbox"
                    aria-label="仅看已启用工具"
                    checked={enabledToolsOnly}
                    onChange={(event) => setEnabledToolsOnly(event.target.checked)}
                  />
                  <span>仅看已启用</span>
                </label>
                <span className="ui-muted text-xs">显示 {filteredTools.length} / {registeredTools.length} 个工具</span>
                <div className="chat-preference-tool-bulk-actions">
                  <button
                    className="ui-button-secondary"
                    type="button"
                    disabled={filteredEnableableToolIds.length === 0}
                    onClick={handleEnableFilteredTools}
                  >
                    启用当前结果
                  </button>
                  <button
                    className="ui-button-secondary"
                    type="button"
                    disabled={filteredEnabledToolIds.length === 0}
                    onClick={handleDisableFilteredTools}
                  >
                    禁用当前结果
                  </button>
                </div>
              </div>
              {registeredToolGroups.length > 0 ? (
                <div className="chat-preference-tool-group-list">
                  {registeredToolGroups.map((group) => {
                    const groupEnabledCount = group.tools.filter(isToolEnabled).length;
                    return (
                      <div key={group.id} className="chat-preference-tool-group">
                        <div className="chat-preference-tool-group-title">
                          <span>{group.label}</span>
                          <span>{groupEnabledCount} / {group.tools.length}</span>
                        </div>
                        <div className="chat-preference-tool-group-tools">
                          {group.tools.map((tool) => {
                            const toolDisplayName = getToolDisplayName(tool);
                            const configured = isModelToolConfigured(tool, webSearchSettings);
                            const disabledReason = tool.id === TAVILY_SEARCH_TOOL_ID && !configured ? "请先配置 Tavily API Key" : "";
                            return (
                              <label key={tool.id} className="chat-preference-network-type-chip">
                                <input
                                  type="checkbox"
                                  aria-label={`启用工具 ${toolDisplayName}`}
                                  checked={isToolEnabled(tool)}
                                  disabled={!configured}
                                  onChange={(event) => handleToolToggle(tool.id, event.target.checked)}
                                />
                                <span>{toolDisplayName}</span>
                                {disabledReason ? <span className="ui-muted text-xs">{disabledReason}</span> : null}
                              </label>
                            );
                          })}
                        </div>
                      </div>
                    );
                  })}
                </div>
              ) : (
                <p className="ui-muted text-xs">暂无符合条件的工具</p>
              )}
            </fieldset>
        </div>
      </PreferenceSection>

      <PreferenceSection
        title="高级参数"
        summary="请求、上下文与工具运行参数"
        expanded={expandedSections.has("advanced")}
        onToggle={() => toggleSection("advanced")}
      >
        <div className="chat-preference-grid">
          <GlobalPreferenceNumberInput
            label="AI 请求失败重试次数"
            value={chatPreferences.aiRequestRetryCount}
            min={0}
            max={20}
            step={1}
            onChange={(value) => void updateChatPreferences({ aiRequestRetryCount: value })}
          />
          <GlobalPreferenceNumberInput
            label="最大工具决策轮次"
            value={chatPreferences.browserAutomationMaxToolIterations}
            step={1}
            onChange={(value) => void updateChatPreferences({ browserAutomationMaxToolIterations: value })}
          />
          <GlobalPreferenceNumberInput
            label="temperature"
            value={chatPreferences.temperature}
            min={0}
            max={2}
            step={0.1}
            onChange={(value) => void updateChatPreferences({ temperature: value })}
          />
          <GlobalPreferenceNumberInput
            label="最大聊天上下文（token）"
            value={chatPreferences.maxTokens}
            min={1}
            max={1_000_000}
            step={1}
            onChange={(value) => void updateChatPreferences({ maxTokens: value })}
          />
          <GlobalPreferenceNumberInput
            label="自动压缩阈值（%）"
            value={chatPreferences.contextCompressionThresholdPercent}
            min={1}
            max={100}
            step={1}
            onChange={(value) => void updateChatPreferences({ contextCompressionThresholdPercent: value })}
          />
          <GlobalPreferenceNumberInput
            label="Network/JS 详情池保留上限"
            value={chatPreferences.toolDetailPoolKeepLimit}
            step={1}
            onChange={(value) => void updateChatPreferences({ toolDetailPoolKeepLimit: value })}
          />
          <GlobalPreferenceNumberInput
            label="top_k"
            value={chatPreferences.topK}
            min={1}
            step={1}
            onChange={(value) => void updateChatPreferences({ topK: value })}
          />
        </div>
      </PreferenceSection>
    </section>
  );
}

interface PreferenceSectionProps {
  title: string;
  summary: string;
  expanded: boolean;
  onToggle: () => void;
  children: ReactNode;
}

function PreferenceSection({ title, summary, expanded, onToggle, children }: PreferenceSectionProps) {
  const contentId = useId();
  const summaryId = useId();

  return (
    <section className="chat-preference-section">
      <button
        className="chat-preference-section-toggle"
        type="button"
        aria-label={`${expanded ? "收起" : "展开"}${title}`}
        aria-expanded={expanded}
        aria-controls={contentId}
        aria-describedby={summaryId}
        onClick={onToggle}
      >
        <span className="chat-preference-section-heading">
          <span className="chat-preference-section-title">{title}</span>
          <span id={summaryId} className="chat-preference-section-summary">{summary}</span>
        </span>
        <span className="chat-preference-section-chevron" aria-hidden="true" />
      </button>
      <div id={contentId} className="chat-preference-section-content" role="region" aria-label={title} hidden={!expanded}>
        {children}
      </div>
    </section>
  );
}

interface PreferenceSwitchProps {
  checked: boolean;
  label: string;
  onChange: (checked: boolean) => void;
}

function PreferenceSwitch({ checked, label, onChange }: PreferenceSwitchProps) {
  return (
    <label className="chat-preference-switch">
      <input
        className="chat-preference-switch-input"
        type="checkbox"
        checked={checked}
        onChange={(event) => onChange(event.target.checked)}
      />
      <span className="chat-preference-switch-control" aria-hidden="true">
        <span className="chat-preference-switch-thumb" />
      </span>
      <span className="chat-preference-switch-label">{label}</span>
    </label>
  );
}

function getToolDisplayName(tool: ModelToolRegistryEntry): string {
  return tool.groupId === "mcp_remote" ? (tool.displayName ?? tool.name) : tool.name;
}

function getTextLengthLabel(value: string): string {
  const length = value.trim().length;
  return length > 0 ? `${length} 字` : "未配置";
}

function getOptionLabel<T extends string>(options: Array<{ value: T; label: string }>, value: T): string {
  return options.find((option) => option.value === value)?.label ?? value;
}
